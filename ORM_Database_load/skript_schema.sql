...
GO
CREATE OR ALTER PROCEDURE [dbo].[FZmenaStavuUzivatele] 
	@uID INT, 
	@status varchar(15)
AS
BEGIN TRY
	UPDATE [User] SET status = @status WHERE userID = @uID;
END TRY
BEGIN CATCH
	PRINT 'Wrong status inserted';
END CATCH
GO
...
GO
CREATE OR ALTER PROCEDURE [dbo].[PUpravitPlaylist]
	@skladba INT,
	@playlist INT
AS
BEGIN
...
GO
CREATE OR ALTER PROCEDURE [dbo].[PZaslaniEmailu]
AS
BEGIN

	DELETE FROM Payment WHERE datetopay < GETDATE() - year(5)
